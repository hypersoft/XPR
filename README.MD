...
word, with no closure for the fact = (+/- = plus or minus [the value of the following fact/fiction/ideal = (+/- = plus or minus [the value of any following suffix = (+/- = plus or minus)])=random-(verb-function=word-hashing)]):
...
Modification-logically-means: not the fact is the sum. Some words start with void-fixes 
