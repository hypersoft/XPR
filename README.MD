...
Lack of certainty is not logical. [=: no-certainty = (not + logical) = negative-sum-operation = (subtraction as an addition = false-operation: fault = break: condition: false-fact-calculations: fact + fiction = fiction (through infinity=(no-meaning-state: forever))]
</small>
