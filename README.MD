...
Lack of certainty is not logical. [=: no-certainty = (not + logical) = negative-sum-operation (if: subtraction as an addition: = false-operation: fault = break: condition: false-fact-calculations: fact + fiction = fiction (through infinity=(no-(meaning-state = known-valuation) = no contract for any semblance of a fact: forever): sum = (false not: true) = negative-calculation)]
</small>

For good-measure, here is a correct-conveyance for the logic of the foregoing-terms:

    (1-o) system, contrivance, plan, software, methods, operations, (etc. = ...)
    (1-e) fixation 
    (2)   sovereign
